...
4. **Provenance note (once).** Above the first injected policy `##` section, add a single visible
   italic line naming this skill and listing the sections it generated, e.g. `_The Verification,
   Git Policy, and File Organization sections below were generated by the `setup-aiengineering`
   skill._` — list only the sections actually injected. **Idempotent:** if a line containing
   "generated by the `setup-aiengineering` skill" already exists, update its section list instead
   of adding a second note. Covers only this skill's injected policy sections; delegated doc-system
...
- Provenance note added/updated (the visible italic line naming the skill above the policy sections).
...
- Stamp one visible italic provenance note above the injected policy sections naming the
  `setup-aiengineering` skill and listing the sections it generated; idempotent (update the list,
  never add a second note). Distinct from the global no-self-attribution rule — this is
  user-requested skill provenance, not model attribution.
