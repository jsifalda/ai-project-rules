...
| [`grill-me`](skills/grill-me/SKILL.md) | Interview the user relentlessly about a plan, design, or decision until reaching shared understanding, working the design tree in rounds and asking every unblocked question in each round. | — | [mattpocock/skills](https://github.com/mattpocock/skills) |
...
| [`merge`](skills/merge/SKILL.md) | `/merge`, or merge main into this branch — fetch the remote default branch, merge it into the current branch (never rebase), resolve conflicts, run the project's own gates, push only the current branch to its own PR/MR. Worktree-safe, GitHub and GitLab, no provider CLI. | — | — |
...
| [`op`](skills/op/SKILL.md) | Route each task in a plan to the cheapest capable Claude model (Haiku for chores, Sonnet by default, Opus for deep reasoning), then execute by dispatching tasks as subagents on their assigned model while the session model only orchestrates. | — | — |
...
| [`setup-aiengineering`](skills/setup-aiengineering/SKILL.md) | Bootstrap a repo's AI-engineering baseline — inject verification/git/file-org policy blocks (plus an opt-in PRD gate) into AGENTS.md/CLAUDE.md, delegate ADRs/changelog/user-scenarios/TODO-backlog to their setup skills, and scaffold a worktree bootstrap hook plus a detected `.worktreeinclude`. For UI repos, writes a `DESIGN.md` source of truth for how the app looks (backfilled from existing UI code, or authored with the user), plus a rule block and a Design-in-sync gate that keep it enforced and current. Keeps that agent file to directives and read-first pointers only — documentation goes to README.md/ARCHITECTURE.md/`docs/adr/`, and a re-run offers to relocate any that drifted back in. Versioned and re-runnable — a coverage self-audit reports uncovered baseline concerns, and re-runs upgrade older setups to the current version. Detects pre-commit hooks, so a gate the hook already runs is not repeated by hand. Stack-agnostic. | `setup-adrs`, `setup-changelog`, `setup-user-scenarios`, `setup-todo-backlog`, `find-skills` (optional), `code-review-nuclear` (optional), `frontend-design` (optional), `hallmark` (optional) | — |
...
| [`ship-pr`](skills/ship-pr/SKILL.md) | `/ship-pr`, or the ship step of a skill that depends on it — go from a dirty working tree, or a committed branch, to an open PR/MR (self-assigned to you) in one pass. | — | — |
