...
| [`review-instructions`](skills/review-instructions/SKILL.md) | Rewrite the project's agent instruction file (`CLAUDE.md`/`AGENTS.md`) as the operating manual a weaker model needs — conventions, named failure modes each with the rule that prevents it, a quality bar of checkable criteria with no adjectives, and exact if-then escalation rules. Drafts beside the original, replaces only on approval. Slash-only. | — |
