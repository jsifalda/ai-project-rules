...
description: Bootstrap a repo's AI-engineering baseline — injects policy blocks (verification gates for lint/typecheck/test/coverage/review/docs-alignment, git policy, file organization, optional PRD gate) into AGENTS.md/CLAUDE.md, scaffolds a root README with a goal statement and a Documentation index, delegates doc systems to the setup-adrs, setup-changelog, and setup-user-scenarios skills, and scaffolds a worktree auto-bootstrap hook plus a detected .worktreeinclude. Stack-agnostic — detects build/test commands per repo (Node, Python, Go, Rust, config/IaC) and degrades gracefully when none exist. Use when the user says "set up ai engineering", "scaffold best practices in this repo", "apply my engineering standards here", "bootstrap agent instructions", or runs /setup-aiengineering. Do NOT use to author a single ADR or changelog entry, to edit policy sections one-off, or to set up only one sub-system (call that setup skill directly).
---
...
| Project README (goal statement + `## Documentation` index) | scaffold (`references/readme-guide.md`) |
...
- **Older version** → the repo is behind in two distinct ways. Before the module menu, tell the user
  both and offer to fix them:
  - **Missing** — every concern whose `Since` is newer than `vN`. Offer to inject those.
  - **Outdated** — every concern whose `(revised vM)` marker is newer than `vN`. Its block is present
    but stale; diff those specific blocks against their current templates and offer to refresh them.

  Then refresh any other block that visibly drifted. Diff and ask per section; preserve local edits;
  never clobber. Continue with the menu for anything still unselected.
...
### Step 8: Project README and docs index

If chosen. Follow `references/readme-guide.md`. This step runs **after** the Step 6 delegation by
design — only now is it known which doc systems actually landed and at which paths, so the index
links what exists rather than what was selected.

The README does two jobs and only two: it states **what the project is for**, and it **indexes the
docs**. Tech stack, dev commands, and project structure belong in `AGENTS.md` — do not duplicate them.

1. **Capture the goal.** Walk the cascade in `readme-guide.md` (`AGENTS.md` `Project Overview` →
   existing README → manifest `description` → entry points → ask outright) and stop at the first hit.
   Present the goal statement plus 2–4 sentences of context and **have the user confirm or correct it
   before writing**, so the README never ships an unconfirmed guess. The one exception is a goal read
   straight from an existing README — those are already the user's own words, and the merge path
   forbids overwriting them, so there is nothing to confirm. The `(inferred)` marker from
   `references/backfill-guide.md` goes in what you show the user, never into README prose.
2. **Resolve the index candidates** by probing each path on disk (`AGENTS.md`, `ARCHITECTURE.md`, the
   ADR dir the delegate chose, `changelog/`, `docs/user-scenarios.md`). Never link an unverified path;
   drop any candidate that does not exist.
3. **Scaffold or merge.**
   - **No root `README.md`** → write the scaffold (title, goal, context, `## Documentation`).
   - **README exists** → never clobber. Append `## Documentation` if missing; if present, add only
     the candidates not already linked **anywhere in the file**, in canonical order. Never remove,
     reorder, or reword an existing bullet. Never overwrite an existing goal statement — if it
     contradicts the goal captured this run, show both and **ask**.
   - **Nothing to add** → no write; report the index is already current.
4. **Section ownership.** This module owns `## Documentation` only. Never create, edit, or remove
   `## PRDs` (owned by `prd-creator`) or `## Codebase Guide` (owned by `create-codebase-docs`), and
   never mirror their links into `## Documentation`.
...
- Whether `AGENTS.md` / `ARCHITECTURE.md` were backfilled from the implementation or left as
  templates.
- Project README: scaffolded, merged (naming which links were added), already current, or skipped —
  and whether the goal statement was inferred-and-confirmed or asked outright.

### Step 9b: Coverage self-audit
...
### Step 10: Offer the Claude GitHub App (Claude Code + GitHub only)
...
- Versioned re-runs — on re-run, read the provenance note's stamped version; if older than the
  current skill version, offer the concerns whose `Since` is newer (missing) and refresh the blocks
  whose `(revised vN)` marker is newer (outdated), plus any other drifted block — diff-and-ask, never
  clobber. A new baseline gap is fixed by adding a checklist row and bumping the version, not by
  patching one repo; an amended block is recorded with `(revised vN)`, never by bumping its `Since`.
- The README module (Step 8) runs after delegation so its index links only doc systems that actually
  landed. Never clobber an existing README — append `## Documentation` if absent, otherwise merge in
  only the links missing from the whole file, and never overwrite an existing goal statement without
  asking. Confirm the goal statement with the user before writing; never ship an unconfirmed guess,
  and never leak the `(inferred)` marker into README prose.
- The README module owns `## Documentation` only — never create, edit, or remove `## PRDs`
  (`prd-creator`) or `## Codebase Guide` (`create-codebase-docs`), and never mirror their links.
- Always end with the Step 9b coverage self-audit against `baseline-checklist.md`; surface every
...
- The GitHub-App offer (Step 10) is Claude-Code-only (gated on `CLAUDECODE=1`) and GitHub-only.
...
- `references/readme-guide.md` — goal-capture cascade, README scaffold, `## Documentation` index
  candidates, and the merge-not-clobber algorithm for an existing README.
- `references/baseline-checklist.md` — canonical baseline concerns + skill version; drives the Step 9b
