...
| [`better-plan`](skills/better-plan/SKILL.md) | Chained planning ritual: enhance the request via prompt-enhancer, build a plan (plan-mode rigor), stress-test it via grill-me, then cost-route tasks via op; runs inside plan mode, so the routed plan lands in the plan file and goes through the native approval gate before it executes, then recaps the run. Slash-only. | `grill-me`, `op`, `prompt-enhancer` | — |
...
| [`loop-todos`](skills/loop-todos/SKILL.md) | Manual `/loop-todos` only — starts a self-cancelling recurring loop that claims one open backlog entry per firing, implements it, verifies it against the host project's own gates, closes it, updates stale docs, and commits locally, never pushing. | `loop`, `setup-todo-backlog` | — |
