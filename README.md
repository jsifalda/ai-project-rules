...
| [`code-review-full`](skills/code-review-full/SKILL.md) | Runs independent reviews concurrently against one pinned diff (correctness, structure, a direct read, Jira spec conformance, and security), verifies every claim against real code and drops the false ones, triages survivors through a council, then reports every finding that survives, grouped by verdict, plus paste-ready comments and one offline HTML report that opens automatically — finally offering to post those comments to the MR or PR one at a time, each only if you approve it. The security lens uses the host's `security-review` skill when one is available and runs inline otherwise. Never edits, commits, or pushes. Slash-only. | `code-review-nuclear`, `council` | — |
...
| [`domain-modeling`](skills/domain-modeling/SKILL.md) | Build and sharpen a project's domain model — resolve terms into `CONTEXT.md` and record ADRs as you design. | — | [mattpocock/skills](https://github.com/mattpocock/skills) |
